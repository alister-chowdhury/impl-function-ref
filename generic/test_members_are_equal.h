...
private:
    ...
    constexpr static bool cmp_eq_light_impl(const T& a, const T& b) {
        return a == b;
    }
    template<typename U=T, std::enable_if_t<!is_simple_type<U>::value, bool> = true>
    constexpr static bool cmp_eq_light_impl(const T& a, const T& b) {
    ...
    template<typename U=T, std::enable_if_t<is_simple_type<U>::value, bool> = true>
    constexpr static bool cmp_eq_normal_impl(const T& a, const T& b) {
        return true;
    }
    template<typename U=T, std::enable_if_t<!is_simple_type<U>::value, bool> = true>
    constexpr static bool cmp_eq_normal_impl(const T& a, const T& b) {
    ...
    struct cmp_eq_size_impl {
        constexpr static bool call(const U&, const U&) {
            return true;
        }
    };
    ...
    struct cmp_eq_size_impl<U, void_t<decltype( std::declval<const U&>().size() )> > {
        constexpr static bool call(const U& a, const U& b) {
            return a.size() == b.size();
        }
    };

    constexpr static bool cmp_eq_size(const T& a, const T& b) {
        return cmp_eq_size_impl<T>::call(a, b);
    }


public:
    constexpr static bool cmp_eq_light(const T& a, const T& b) {
        return (
            cmp_eq_light_impl<>(a, b)
            && cmp_eq_size(a, b)
        );
    }
    constexpr static bool cmp_eq_normal(const T& a, const T& b) {
        return (
...
private:
    ...
    struct all_true_impl {
        constexpr static bool call(...) { return true; }
    };
    template <typename BoolT, typename... NextBoolsT>
    struct all_true_impl<BoolT, NextBoolsT...> {
        constexpr static bool call(const BoolT current, const NextBoolsT... nexts) {
            return current & all_true_impl<NextBoolsT...>::call(nexts...);
        }
    };
    template <typename... NextBoolsT>
    constexpr static bool all_true(const NextBoolsT... bools) {
        return all_true_impl<NextBoolsT...>::call(bools...);
    }


public:
    constexpr static bool call(const Ts&... a_values, const Ts&... b_values) {
        return (
